...
export const fetchImagesUrl = async (): Promise<ImageData[] | null> => {
  try {
    const response = await fetch("http://localhost:3000/categories/Portrait");
    if (response.ok) {
      const data = await response.json();

      const images: ImageData[] = data.images.map((image: any) => ({
        id: image._id,
        url: image.url,
        alt: image.alt,
        width: image.width,
...
export const deleteImage = async (id: string) => {
  try {
    const response = await fetch(
      `http://localhost:3000/categories/Portrait/image/${id}`,
      {
        method: "DELETE",
      }
    );
...
export const AddImage = async (file: File) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/categories/Portrait/image/`,
      {
        method: "POST",
        body: formData,
      }
